    ...
    if let Ok(lines) = read_lines("./src/year2021/data/day6input.txt") {
        ...
        for (line_num, line) in lines.enumerate() {
            if let Ok(contents) = line {
                contents.split(",").map(|s| s.parse().unwrap()).for_each(|x| fish.push(x));
            }
        }
        ...
        for day_num in 1..=days {
            let now = Instant::now();
            let new_fish: i32 = fish.par_iter_mut().map(|f| {
                if *f == 0 {
                    *f = 6;
                    1
                } else {
        ...
        println!("Iter spent {} seconds calculating", start.elapsed().as_secs_f64());
    }
}
...
pub fn fish_helper(fish: &mut Vec<i32>, curr_day: usize, last_day: usize) -> usize {
    ...
    let new_fish: i32 = fish.iter_mut().map(|f| {
        if *f == 0 {
            *f = 6;
            1
        } else {
    ...
    if curr_day < 6 {
        ...
        let (a, b) = rayon::join(|| fish_helper(&mut left, curr_day + 1, last_day), || fish_helper(&mut right, curr_day + 1, last_day));
        a + b
    } else {
...
pub fn calc_fish_recurse(days: usize) {
    ...
    if let Ok(lines) = read_lines("./src/year2021/data/day6input.txt") {
        ...
        println!("Recursive spent {} seconds calculating", start.elapsed().as_secs_f64());
    }
}
...
pub fn calc_fish_buckets(days: usize) {
    if let Ok(lines) = read_lines("./src/year2021/data/day6input.txt") {
        ...
        for (line_num, line) in lines.enumerate() {
            if let Ok(contents) = line {
                contents.split(",").map(|s| s.parse().unwrap()).for_each(|x: usize| fish[x] += 1);
            }
        }
